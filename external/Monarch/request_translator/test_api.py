...
load_dotenv()
MONARCH_REQUEST_TRANSLATOR_URI = os.getenv(
    "MONARCH_REQUEST_TRANSLATOR_URI", "http://127.0.0.1:5000"
)
...
def delete_monitoring_request(request_id):
    ...
    delete_url = (
        f"{MONARCH_REQUEST_TRANSLATOR_URI}/api/monitoring-requests/delete/{request_id}"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Test the monitoring endpoint of the Flask API"
    )
    parser.add_argument(
        "action",
    ...
    parser.add_argument(
        "--json_file",
        default="requests/request_nf.json",
        help="Path to the JSON file (only for submit action)",
    )
    parser.add_argument(
        "--request_id", help="ID of the request to delete (only for delete action)"
    )
